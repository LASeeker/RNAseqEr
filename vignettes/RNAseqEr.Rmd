...
```{r, results='hide', fig.keep='all', fig.width= 8, fig.height=5}
cns <- seurat_proc(cns)
...
keep_res <- max_pure(save_path = here("outs", "all_celltypes","tables", "cluster_purity_data"))
...
here("..")
cns <- annotate_seqEr(cns, ident_use = keep_res)
...
"average" that does express all the typical cell lineage markers but nothing special over 
