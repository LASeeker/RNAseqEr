...
metadata column can be used for subsetting by providing it as argument 
"subset_column" . subset_RNAseqEr will save each cell lineage data in 
a separate .RDS file.
...
We show here our approach using the example of oligodendrocytes. 
...
approach is to decide by looking at the results of differential gene expression 
...
is negative for markers that characterize other clusters.

So all potentially interesting resolutions can be used below:

```{r, eval = FALSE}

# Automatically select resolutions that changed clustering
int_res <- select_res(seur, col_pattern = "RNA_snn_res.")
...
## Data Validation and Safeguards for Small Datasets

RNAseqEr includes comprehensive safeguards for small datasets to prevent analysis failures:

### **Dataset Size Analysis**
The workflow automatically analyzes your dataset size and provides recommendations:

```{r, eval = FALSE}
# Analyze dataset size and get recommendations
size_analysis <- analyze_dataset_size(seur_obj, verbose = TRUE)
```

### **Cluster Size Validation**
Before performing differential expression analysis, the workflow validates cluster sizes:

```{r, eval = FALSE}
# Validate cluster sizes for a specific resolution
validation <- validate_cluster_sizes(seur_obj, "RNA_snn_res.0.8", 
                                   min_cells_per_cluster = 3,
                                   min_cells_for_comparison = 5)
```

### **Automatic Filtering**
If clusters have too few cells, they are automatically filtered out:

```{r, eval = FALSE}
# Filter out small clusters
filtered_obj <- filter_small_clusters(seur_obj, "RNA_snn_res.0.8", 
                                     min_cells_per_cluster = 3)
```

### **Safe Differential Expression**
The workflow uses safe differential expression analysis with error handling:

```{r, eval = FALSE}
# Safe differential expression with validation
safe_result <- safe_differential_expression(seur_obj, "RNA_snn_res.0.8",
                                          min_cells_per_cluster = 3,
                                          min_cells_for_comparison = 5)
```

### **Size-Based Parameter Adjustment**
For very small datasets (< 100 cells), the workflow automatically adjusts parameters:
...
### **Error Handling**
The workflow gracefully handles analysis failures and provides informative messages about why certain steps were skipped.
...
```{r, eval = FALSE, fig.height= 50, fig.width=8}
library(here)
...
```{r, eval = FALSE, plot.width = 8, plot.height = 30}
...
The function supports two levels of p-value adjustment:
1. **Primary adjustment**: Seurat's Bonferroni correction within each comparison
2. **Additional adjustment**: Optional correction across all comparisons (e.g., BH method)
...
You can also generate reports manually for specific analyses:

```{r, eval = FALSE}
# For cell lineage analysis
generate_report(seur_obj = seur,
               save_dir = "../",
               dir_lab = "Oligodendrocytes",
               workflow_params = list(min_cells_per_cluster = 3),
               researcher_name = "Researcher Name",
               project_title = "Oligodendrocyte subclustering analysis",
               tissue_type = "Brain")
```
...
You can also generate manuscripts from existing analysis results without running the full workflow:
...
```{r, eval = FALSE}
# Simplified manuscript generation with auto-detection
...
if(validation$ready) {
  ...
  cat("Missing prerequisites. Recommendations:\n")
...
### Remove Clusters
```{r, eval = FALSE}
# Remove specific clusters from analysis
filtered_obj <- remove_clu(seur_obj = seur,
                           clusters_to_remove = c("cluster_1", "cluster_2"),
                           cluster_col = "cluster_id")
```

### Select Resolution
```{r, eval = FALSE}
# Automatically select optimal clustering resolution
selected_res <- select_res(seur_obj = seur,
...
### Silhouette Plot
```{r, eval = FALSE}
# Generate silhouette plots for cluster quality assessment
sil_plot(seur_obj = seur,
         cluster_col = "cluster_id",
         save_dir = "../")
```
...
```{r, eval = FALSE}
# Analyze cluster purity at different resolutions
cluster_purity(seur_obj = seur,
               reduction = "umap",
               col_pattern = "RNA_snn_res.",
               save_dir = "../")
```
...
### Fine Annotation
```{r, eval = FALSE}
# Perform fine-grained cell type annotation
fine_annotate(seur_obj = seur,
              cluster_col = "cluster_id",
              tissue_ref = "Brain")
```

### Condense Marker Lists
```{r, eval = FALSE}
# Condense multiple marker gene lists
condensed_markers <- condense_marklists(marker_lists = list(markers1, markers2),
                                       top_n = 10)
```

### Minimum Difference Heatmap
```{r, eval = FALSE}
# Create heatmaps showing minimum differences between clusters
minimum_diff_heatmap(seur_obj = seur,
                     cluster_col = "cluster_id",
                     genes = marker_genes,
                     save_dir = "../")
```
