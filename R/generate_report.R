...
generate_methods_report <- function(seur_obj, save_dir, dir_lab, workflow_params, 
  ...
  methods_text <- c(
    ...
    "",
    "### 2. Data Validation and Small Dataset Safeguards",
    paste("- Dataset size analysis was performed to assess statistical power"),
    paste("- Total cells analyzed:", n_cells),
    paste("- Dataset size category:", if(n_cells < 100) "very small" else if(n_cells < 1000) "small" else if(n_cells < 10000) "medium" else "large"),
    paste("- Cluster size validation was performed before differential expression analysis"),
    paste("- Minimum cells per cluster threshold:", workflow_params$min_cells_per_cluster %||% 3),
    paste("- Minimum cells for pairwise comparisons:", workflow_params$min_cells_for_comparison %||% 5),
    if(n_cells < 100) {
      paste("- Very small dataset detected: parameters adjusted for reliability")
    } else {
      paste("- Dataset size adequate for standard analysis parameters")
    },
    paste("- Small clusters with insufficient cells were automatically filtered out"),
    paste("- Error handling was implemented for all differential expression analyses"),
    "",
    ...
    "",
    "### 4. Cluster Resolution Selection",
    ...
    "",
    "### 9. Statistical Testing and Multiple Testing Correction",
    ...
    "",
    "### 12. Visualization and Interactive Analysis",
...
generate_decision_justifications <- function(seur_obj, save_dir, dir_lab, 
  ...
  decisions_text <- c(
    ...
    "",
    "### 1. Dataset Size Assessment and Validation",
    paste("**Decision:** Analyzed dataset size and applied appropriate safeguards"),
    paste("**Dataset characteristics:**", ncol(seur_obj), "cells,", nrow(seur_obj), "genes"),
    paste("**Size category:**", if(ncol(seur_obj) < 100) "very small" else if(ncol(seur_obj) < 1000) "small" else if(ncol(seur_obj) < 10000) "medium" else "large"),
    paste("**Validation thresholds:** Minimum", workflow_params$min_cells_per_cluster %||% 3, "cells per cluster, minimum", workflow_params$min_cells_for_comparison %||% 5, "cells for pairwise comparisons"),
    "**Justification:** Small datasets require special consideration to ensure statistical reliability. Cluster size validation prevents analysis failures and ensures meaningful results. For very small datasets (< 100 cells), parameters were automatically adjusted to prioritize reliability over sensitivity.",
    "",
    ...
    "",
    "### 3. Clustering Resolution Selection",
    ...
    "",
    "### 4. Cell Type Annotation Strategy",
    ...
    "",
    "### 6. Differential Expression Analysis Parameters",
    ...
    "",
    "### 8. Gene Ontology Analysis Settings",
